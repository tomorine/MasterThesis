...
def main():
    ...
    hi = int(argv[3]) # circuit high
    wd = int(argv[2])  # circuit wide
    outermost = wd*2 + hi*2 - 4
    s = Solver() # sは制約式の集合
    ...
    clock_num = [Int("clock_num[%d]" % (i)) for i in range(circ.op_num)]
    wire_id = [[[Int("wire_id[%d][%d][%d]" %  (k, j, i)) for i in range(hi)]for j in range(wd)] for k in range(outermost)]
    one_connect = [[[Int("one_connect[%d][%d][%d]" % (k, j ,i)) for i in range(hi)] for j in range(wd)] for k in range(outermost)]
    length = [Int("length[%d]" % (i)) for i in range(circ.op_num)]
    # connect_num = [[Int("connect_num[%d][%d]" % (j, i)) for i in range(1, hi-1)] for j in range(1, wd-1)]
    ...
    for k in range(hi):
        for j in range(wd):
            ...
            for i in range(circ.op_num):
                ...
                tmplist.append(op_exist[i][j][k])
                tmplist2.append(op_exist[i][j][k])
            s.add(Sum([tmp for tmp in tmplist]) <= 2)
            s.add(Sum([tmp for tmp in tmplist2]) <= 1)
    ...
    for i in range(wd):
        for j in range(hi):
            for node in range(circ.op_num):
                s.add(Implies(op_exist[node][i][j]==1, clock_num[node] == clock_zone[i][j]))
    ...
    """
    # wire_idが一定範囲の整数を取る制約
    for i in range(wd):
        for j in range(hi):
            for id in range(outermost):
                s.add(one_connect[id][i][j] >= 0, one_connect[id][i][j] <= 2)
            for id in range(outermost):
                s.add(wire_id[id][i][j] >= 0, wire_id[id][i][j] <= 1)
    # 一番外側のクロックゾーンがすべて違うwire_idを持つ制約
    for id  in range(outermost):
        wire_id_list = []
        for i in range(wd):
            wire_id_list.append(wire_id[id][i][0])
            wire_id_list.append(wire_id[id][i][hi-1])
        for j in range(hi):
            wire_id_list.append(wire_id[id][0][j])
            wire_id_list.append(wire_id[id][wd-1][j])
        s.add(Sum([tmp for tmp in wire_id_list]) <=1)
    # 内側のクロックゾーンは周り８つのどれかのwire_idと同じになる
    for i in range(1, wd-1):
        for j in range(1, hi-1):
            for id in range(outermost):
                s.add(Implies(wire_id[id][i][j] == 1, Or(wire_id[id][i+1][j+1] == 1,
                                                          wire_id[id][i+1][j-1] == 1,
                                                          wire_id[id][i-1][j+1] == 1,
                                                          wire_id[id][i-1][j-1] == 1) == True))
                s.add(Implies(clock_zone[i][j] != clock_zone[i+1][j+1], And(wire_id[id][i][j]==1, wire_id[id][i+1][j+1]==1) == False))
                s.add(Implies(clock_zone[i][j] != clock_zone[i+1][j-1], And(wire_id[id][i][j]==1, wire_id[id][i+1][j-1]==1) == False))
                s.add(Implies(clock_zone[i][j] != clock_zone[i-1][j+1], And(wire_id[id][i][j]==1, wire_id[id][i-1][j+1]==1) == False))
                s.add(Implies(clock_zone[i][j] != clock_zone[i-1][j-1], And(wire_id[id][i][j]==1, wire_id[id][i-1][j-1]==1) == False))
    # 接続数１のwire_idは１つまで
    for id in range(outermost):
        ...
        for i in range(1, wd-1):
            for j in range(1, hi-1):
                tmplist = []
                idlist.append(one_connect[id][i][j])
                tmplist.append(wire_id[id][i+1][j+1])
                tmplist.append(wire_id[id][i+1][j-1])
                tmplist.append(wire_id[id][i-1][j+1])
                tmplist.append(wire_id[id][i-1][j-1])
                s.add(Implies(And(wire_id[id][i][j] == 1, Sum([tmp for tmp in tmplist]) == 1), one_connect[id][i][j]== 1))
                s.add(Implies(wire_id[id][i][j] == 1, Sum([tmp for tmp in tmplist]) != 0))
        s.add(Sum([tmp for tmp in idlist]) <= 1)
    """
    for i in range(1, wd-1):
        for j in range(1, hi-1):
            s.add(Or(clock_zone[i][j]==clock_zone[i+1][j+1],
                     clock_zone[i][j]==clock_zone[i+1][j-1],
                     clock_zone[i][j]==clock_zone[i-1][j+1],
                     clock_zone[i][j]==clock_zone[i-1][j-1],
            ) == True)
    ...
    def search_length(sonode, tonode, tmplist):
        ...
        if sonode.gatetype == "p_input":
            tmplist.append(clock_num[sonode.id])
            return (tmplist)
        else :
            tmplist.append(1)
    ...
    # todo : clock_wireを通すようにクロックゾーンを配置する
    """
    # スループットを向上させる制約
    for node in range(circ.op_num):
        tmplistlist = []
        if len(circ.find_node_id(node).input) >= 2:
            for sonode in circ.find_node_id(node).input:
                ...
                for i in range(wd):
                    for j in range(hi):
                        tmplist.append(wire_exist[sonode.id][node][i][j])
                s.add(length[node] == Sum([tmp for tmp in tmplist]))
                tmplist.append(length[node])
                tmplistlist.append(tmplist)
            if len(tmplistlist) == 2:
                s.add(Sum([tmp for tmp in tmplistlist[0]]) == Sum([tmp for tmp in tmplistlist[1]]))
    # print model or
